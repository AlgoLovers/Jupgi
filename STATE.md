...
- **2026-07-26 · 렌더 정확성의 기계 검증 도입 — "눈 샘플링"의 시대 종료.**
  버그가 반복된 구조적 원인 = 렌더 가림 순서를 검증하는 자동 장치가 없어 눈으로 몇 각도만
  보고 배포한 것. `ShellStackReferenceTest`가 **픽셀 z-buffer 레퍼런스**와 실전 그리기 순서를
  전 작품 × 진행도 × 카메라 36각도(1,368프레임)에서 자동 비교하고, 실패 시 재현 좌표
  (작품·progress·yaw·pitch)를 출력한다. 이 기계가 도입 즉시 769프레임 오류를 검출했고,
  아래 세 수정으로 **7건(봉인된 알려진 한계)까지 감소**:
  1. **뉴웰(Newell) 순서** — centroid 깊이 정렬은 원리적으로 틀림(접힌 겹은 거울상이라 대각선
     방향이 반대 → 기운 카메라에서 centroid 위치 차이가 오프셋을 압도). 화면에서 실제로
     겹치는(SAT) 삼각형 쌍마다 평면 부호 검사로 간선 → 위상 정렬.
  2. **평면 클러스터 오프셋** — 오프셋은 "같은 평면에 포개진" 셸 사이에서만(순위 기반, 중앙
     대칭). 다른 평면(수직 날개)에 스택 겹 값을 주면 인공 교차가 생기던 문제 제거.
  3. **|각|≤90° 스텝은 겹 재배치 없음** — 세우는 접기는 뒤집어 포개는 게 아니다.
  잔여 7건은 90° 수직 셸+스택 접합의 화가 알고리즘 본질 한계(9~20%, 특정 각도) — 예외 목록으로
  봉인(새 회귀는 즉시 RED), M2 Filament z-buffer 에서 자연 해소. **복잡한 작품(수십~수백 단계)을
  넣을 때도 이 관문이 렌더 정확성을 기계적으로 보증한다.**

- **2026-07-26 · 완성 자세는 "자세 전환 스텝"으로 연출한다.** 비행기 날개 높이가 안 맞아
  보인 것은 렌더링 버그가 아니라 **종이가 테이블에 누워 있어서**였다(위 겹 날개 +z, 아래 겹
  날개 -z — 기하는 실물과 동일). 해법: 전체 정점을 90° 돌리는 "세우기" 스텝을 작품 마지막에
  추가 — 강체 회전이라 FoldInvariants 통과, 재생에서 "비행기를 세웁니다"가 자연스럽게
  애니메이션되고, 세우면 날개가 양옆 수평(같은 높이)이 된다. 전체-이동 스텝은 접기가 아니라
  자세 전환이므로 layer·flipParity 계산에서 제외. 완성 상태(progress=stepCount)에선 가이드선을
  숨긴다(다음 접을 선 안내가 목적이므로).

- **2026-07-26 · 겹 그리기 순서 = "평균 깊이"가 아니라 가림 위상 정렬.**
  기하 오프셋 후에도 대문·부채·비행기에서 포개짐이 남았다(데모만 정상). 원인: 겹 전체의
  평균 깊이로 정렬했는데, 겹들이 화면에서 **부분적으로만 겹치면** 화면 위치가 평균 깊이를
  오염시켜 오프셋(0.016)이 위치 차이(0.12)에 묻힌다 — 대문 오른쪽 문짝이 중앙 뒤로 가던 원인.
  고전 화가 알고리즘의 근본 한계(그래서 z-buffer가 발명됨). 해법: 겹은 평면이므로 두 겹이
  화면에서 겹칠 때 **겹침 지점에서의 평면 깊이**를 비교하면 전 영역에서 부호가 같다 —
  쌍별 비교로 간선을 만들어 **위상 정렬**(Kahn, 사이클은 평균 깊이 폴백). 화면상 안 겹치는
  겹은 간선 없음(순서 무관). 데모·대문·부채·비행기 전부 에뮬 확인.

- **2026-07-26 · 겹 렌더링 = 기하 오프셋(업계 표준 polygon offset 계열)으로 전환.**
  정렬 기반(깊이 양자화 + 겹 순서 키 + 화면 방향 휴리스틱)은 동일평면인 한 보는 방향에 따라
  틀릴 수 있었다 — flipLayers 가 base +z 고정이라 접는 중·옆 각도에서 포개짐이 어긋나던 원인.
  이제 렌더 시점에 각 겹을 쌓임 방향(면 법선 × 뒤집힘 부호)으로 `겹 × PAPER_THICKNESS(0.008)`
  만큼 실제로 띄운다: 깊이가 실제로 갈려 **순수 깊이 정렬만으로 어느 각도에서든 옳다.**
  OpenGL glPolygonOffset·D3D depth bias·데칼 렌더링과 같은 원리, 실제 종이 두께라 물리적으로도
  정확(가장자리에 겹이 비쳐 종이 뭉치 질감). 이전 178° 시도와 달리 **도메인 기하 불변**이라
  힌지가 깨지지 않는다. 양자화·flipLayers 휴리스틱 제거로 렌더러가 단순해짐.
  `flipParity`(면별 뒤집힘 홀짝, progress 함수)를 도메인이 제공.
